...
; 0022 - 12/27/06 - more planet types, array for planet images, read in resource totals
;
; to-do's
...
; x make an F-key toggle a global-ignore-visibility flag
...
If vis$<>"" Then
	...
	Next
End If
;parse out production events
Dim prodevent$(20,3)
Global prodmax=0
For i=1 To 20
	t$=parseturn(resp$,"production"+i) ; e.g. production5=Mars:Pentium:5
	If t$<>"" Then
		prodmax=i
		p1=Instr(t$,":")
		p2=Instr(t$,":",p1+1)
		prodevent(i,1)=Left(t$,p1-1)
		prodevent(i,2)=Mid(t$,p1+1,p2-p1-1)
		prodevent(i,3)=Mid(t$,p2+1)
	End If
Next
;parse out dynsaty totals
Dim resource$(5)
resource(1)=parseturn(resp$,"natrium")
resource(2)=parseturn(resp$,"dysprosium")
resource(3)=parseturn(resp$,"pentium")
resource(4)=parseturn(resp$,"smithore")
resource(5)=parseturn(resp$,"money")
...
Dim stainb(5)
stainb(0)=makecol(240,240,250) ;default
stainb(1)=makecol(180,200,180) ;gray eyeguys
...
While r$<>""
	...
	If Len(l$)>0 And l$<>"[IbexMap]" Then
		...
		If map_e$="planet" Then
			e\etype=EPLANET
			e\owner=Int(map_o$)
			If map_p2$="1" Then e\img=planetimg(1)
			If map_p2$="2" Then e\img=planetimg(2)
			If map_p2$="3" Then e\img=planetimg(3)
			If map_p2$="4" Then e\img=planetimg(4)
			If map_p2$="5" Then e\img=planetimg(5)
			If map_p2$="6" Then e\img=planetimg(6)
...
;production Window
winProd=Window(700,70,220,300,"Production",0,1,0,1,0)
lblProdText=Label(5,5,"",0)
...
prodmsg$="Resource Totals:"+n$+n$+"Funds: $"+resource(5)+n$
prodmsg=prodmsg+"Natrium: "+resource(1)+n$
prodmsg=prodmsg+"Dysprosium: "+resource(2)+n$
prodmsg=prodmsg+"Pentium: "+resource(3)+n$
prodmsg=prodmsg+"Smithore: "+resource(4)+n$
prodmsg=prodmsg+n$+n$+"This Turn:"+n$
For i=1 To prodmax
	Select prodevent(i,2)
	Case "Ship"
		prodmsg=prodmsg+prodevent(i,1)+" built a ship called "+prodevent(i,3)+n$
	Case "NoShip"
		prodmsg=prodmsg+prodevent(i,1)+" did not build a ship because there was no space!"+n$
	Default
		prodmsg=prodmsg+prodevent(i,1)+" produced "+prodevent(i,3)+" units of "+prodevent(i,2)+n$
	End Select
Next

sendmessage(lblProdText,"LM_SETTEXT",prodmsg)
UpdateMap()
...
Repeat
  ...
  UpdateGUI()
				...
				Case imgmap
					...
					If app\mb1=1 Then
						If hexanchorx=-1 Then
							...
							Select h_ok 
								...
								Case 0
									PlaySound bzzt
									mmsg$="You may only select your own piece!"
								Case 2
									PlaySound bzzt
									mmsg$="You've already moved that piece!"
							End Select
						Else
							If pathcount>pmovemax Then
								PlaySound bzzt
								mmsg$="You may not move that far!"
							Else
								If pathcount>0 Then
									h_ok=1
									For e.entity=Each entity
										If e\x=mhx And e\y=mhy And (e\etype=EPLAYER Or e\etype=EPLANET) Then
											PlaySound bzzt
											mmsg$="You may not move to a space occupied by another ship or a planet!"
											h_ok=0
										End If
									Next
									If h_ok=1 Then
					...
					If hexanchorx>=0 And hexanchory>=0 And mhx<PACKBASE And mhy<PACKBASE Then ;do path
					  ...
					  stt=MilliSecs()
						...
						Wend
						el=ste-stt
						sendmessage(lblInstr,"lm_settext",p$+"  steps="+pathcount+"  time="+el+n$+n$+"Right-click to cancel movement."+n$+n$+mmsg$)
						UpdateMap()
						sendmessage(imgmap,"im_setimage",mapimg)
						FlushKeys
					End If
	...
	If KeyHit(65) Then ;f7
		PlaySound thup
		planettextcolor=Not planettextcolor
		updatemap()
		sendmessage(imgmap,"im_setimage",mapimg)
	End If
...
For e.entity=Each entity
	...
	If e\etype=EPLANET And (map_vis(e\x,e\y)>0 Or ignorevis=1) Then
		If planettextcolor=True Then
			setcol(stainb(e\owner))
		Else
			setcol(stainb(0))
		End If
...
FreeImage planetimg(1) ;blue
planetimg(1)=CreateImage(hexa,hexb)
SetBuffer ImageBuffer(planetimg(1))
For i=10 To 1 Step -1
 Color (11-i)*12,0,(11-i)*12+120
 Oval2( hexa/2,hexb/2,i,i,1)
Next

FreeImage planetimg(2) ;yellow
planetimg(2)=CreateImage(hexa,hexb)
SetBuffer ImageBuffer(planetimg(2))
For i=10 To 1 Step -1
 Color (11-i)*12+120,(11-i)*8+120,0
 Oval2( hexa/2,hexb/2,i,i,1)
Next

FreeImage planetimg(3) ;red
planetimg(3)=CreateImage(hexa,hexb)
SetBuffer ImageBuffer(planetimg(3))
For i=10 To 1 Step -1
 Color (11-i)*8+120,0,0
 Oval2( hexa/2,hexb/2,i,i,1)
Next

FreeImage planetimg(4) ;orange
planetimg(4)=CreateImage(hexa,hexb)
SetBuffer ImageBuffer(planetimg(4))
For i=10 To 1 Step -1
 Color (11-i)*12+120,(11-i)*8+80,0
 Oval2( hexa/2,hexb/2,i,i,1)
Next

FreeImage planetimg(5) ;light blue
planetimg(5)=CreateImage(hexa,hexb)
SetBuffer ImageBuffer(planetimg(5))
For i=10 To 1 Step -1
 Color (11-i)*8+50,(11-i)*8+100,(11-i)*8+150
 Oval2( hexa/2,hexb/2,i,i,1)
Next

FreeImage planetimg(6) ;gray
planetimg(6)=CreateImage(hexa,hexb)
SetBuffer ImageBuffer(planetimg(6))
For i=10 To 1 Step -1
 Color (11-i)*8+120,(11-i)*8+120,(11-i)*8+120
 Oval2( hexa/2,hexb/2,i,i,1)
Next
...
Function ReAssociateEntitiesAndImages()
	For e.entity=Each entity
		If e\etype=EPLANET Then e\img=planetimg(e\classtype)
